...
pub enum SpecError {
    ...
    /// An RFC-9 shared-state root observed from an RFC-10 function contract.
    /// Contracts run at function entry/exit, outside any lexical `holding`
    /// acquisition, so the current language has no sound snapshot with which to
    /// interpret this path. Reject it before any Verus/vacuity harness is built.
    SharedStateInContract {
        root: String,
        position: &'static str,
        span: Span,
    },
...
impl fmt::Display for SpecError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SpecError::SharedStateInContract { root, position, .. } => write!(
                f,
                "shared-state root `{root}` may not be observed in `{position}`: contracts are \
                 evaluated outside `holding`, and RFC-10 does not identify entry/exit state \
                 with a later guarded snapshot"
            ),
        }
    }
}
...
struct Validator {
    ...
    in_scheme_step: bool,
    shared_roots: HashSet<String>,
    contract_bound: HashSet<String>,
    contract_position: Option<&'static str>,
}
        ...
        match kind {
            ArgKind::Pred => match arg {
                ...
                Expr::Closure { params, body } => {
                    ...
                    let saved_bound = self.contract_bound.clone();
                    self.contract_bound.extend(params.iter().cloned());
                    self.in_combinator_closure = true;
                    self.walk_expr(body, span);
                    self.in_combinator_closure = saved;
                    self.contract_bound = saved_bound;
                }
                _ => {
