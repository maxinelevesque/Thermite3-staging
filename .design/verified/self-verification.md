...
| REQ-5 (`subsumes` verified + matched) | SHIPPED | `verus_core::subsumes` is proved over the complete 12-atom `u16` hosted bitset (`Read` through RFC-13 `Blocks`; prior bit assignments stay stable and `Blocks=11`; the `(callee & !caller)==0` test is bounded to `< 4096` and matched to the 12-way `spec_subsumes` conjunction) plus three lattice-law `proof fn`s; `thermite_verified::subsumes_masks` (the plain mirror) is consumed by `thermite_lower::effects::subsumes` and matched by the 16777216-pair (4096×4096) exhaustive equivalence test (mechanism (c), AC-4, 0 mismatches). |
