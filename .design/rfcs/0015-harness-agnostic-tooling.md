...
2. A `justfile` materializes a contributor's own choice — `just use claude`,
   `just use crosslink`, `just use day` — from stacks kept in-tree under
   `opt-in/`.
...
### 1.4 The tracked control plane is not self-contained

This is the strongest of the four, because unlike §1.2 and §1.3 it is not
conditional on this fork having installed a second harness. It is what any
contributor gets.

`.gitignore` marks `.claude/hooks/`, `.claude/commands/` and `.claude/mcp/` as
"auto-generated by crosslink init (not project source)". They are ignored, and
absent from a fresh checkout. Meanwhile:
...
* tracked `.mcp.json` declares **three** servers, all named `crosslink-*`, all
  pointing into the absent `.claude/mcp/`.

So a fresh clone carries tracked configuration referencing nine files that do
not ship. A contributor who has Claude Code but not crosslink does not get a
degraded control plane; they get a broken one, silently, and the gate that
exists to audit the control plane reports success.
...
The stacks move under a neutral path — `opt-in/claude/`, `opt-in/crosslink/`,
`opt-in/day/` — where they are ordinary tracked files that no tool reads by
accident, and where the path itself states the claim
`telos/the-clone-is-neutral` makes: everything below it is a choice.

Three stacks, not two. `.mcp.json` declares three `crosslink-*` servers and is
therefore crosslink's, not Claude Code's; `opt-in/day/` stays empty until step 4,
since day's duplicated hooks and its project-scoped MCP entry were already
removed.

### 3.2 `just use`

```
just use claude      # materializes .claude/settings.json and .claude/agents/
just use crosslink   # materializes .crosslink/ and .mcp.json
just use day         # installs day's vocabulary; materializes .claims/
just use             # lists what is available and what is currently installed
```
...
`.claude/settings.json` to the stack that produces it, so a second tool's wiring
...
* **Stack drift.** Once `.claude/settings.json` is generated rather than tracked,
  nothing pins that the generated file matches the stack. The control-plane gate
  checks the materialized result, so a stale stack is caught only for
  contributors who ran the recipe — which is the same class of hole
  `crosslink #93` opened, relocated rather than closed.
...
1. Stacks move to `opt-in/`, `justfile` recipes land, materialized paths
