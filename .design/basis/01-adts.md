...
- **REQ-10 (recursive type → Verus recursive enum; `Box` → `Box`; structural
  ...
  GROUNDED recursive-`List` proof. The overflow-free `nat` representation is
  available only when the `spec fn` declares a fixed-width integer result. A
  structural predicate or traversal retains its declared result type even when
  its body contains a call through a `Box` dereference; syntactic resemblance to
  a numeric fold cannot rewrite `-> bool` (issue #7).
...
| REQ-10 (recursive type → Verus recursive enum; `Box`; structural `decreases`) | SHIPPED | epic **#62** Stage 1c (#67), declared-return correction #7. `lower.rs`: `lower_enum` emits `Cons(u64, Box<List>)` (`lower_type` `Type::Box`→`Box<…>`); a fixed-width-integer `spec fn` of the ADT-fold-sum shape (`is_adt_fold_sum`) lowers `-> nat` with `decreases l` over the datatype VALUE (Verus's built-in structural order) and `Expr::Deref`→`*t`, integer casts coerced `as nat` (`Ctx::nat_ret`). `declared_return_can_lower_to_nat` gates BOTH the emitted return and the program-wide `nat_fns` set, so recursive `all_below(..) -> bool` and non-recursive `calls_it(..) -> bool` retain `bool` while their numeric `depth(..) -> u64` helper retains `nat`. Consumer: `pub fn lower`. Verification: real Verus passes `list_sum_lowers_recursive_box_and_verifies_l3`, `divergence_tree_fold_does_not_lower_and_verify`, and `adt_predicates_keep_their_declared_bool_return`; Forge certifies `Tree`, `all_below`, `depth`, and `calls_it` L3 in `adt_predicates_with_deref_calls_certify_at_their_declared_type`. The `! alloc` effect-row for an exec `Box`-constructor is forward-ready (`effects.rs` accepts `alloc`); the corpus `list_sum` is spec-fn-only (`pure`), so no exec `alloc` is exercised this stage. |
