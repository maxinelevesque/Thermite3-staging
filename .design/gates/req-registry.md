...
audited-content-sha256: 146ee4c56db760c2629bce37d11df29c4449ac148ce30274a54a78f6ae370f10 (re-pinned 2026-08-11 for the RFC-8 effect-algebra requirement register, which added two requirements and amended three. The three RFC-8 requirements — REQ-SYNTAX-EFFECT-BASIS, REQ-SYNTAX-EFFECT-DECLARED, REQ-SPEC-EFFECT-COMMUTATION — each gained `.design/syntax/effect-algebra.md` as a contributor and a doc evidence entry naming the REQs and ACs that contract them, and BASIS's remaining_scope records that the two labels RFC-8 left unplaced are now placed: `term` is state(termios)/{get,put} and `net(d)` is the combination state(d) + io(s_d), so the label map is total over the nine `ast::Effect` variants. REQ-SYNTAX-EFFECT-FOOTPRINT is NEW, and it is introduced by the component contract rather than by RFC-8, which does not name the projection: a row projects to the region instances it may read and those it may write, which the 9-atom mask cannot express because EffectKind::of drops the region argument. Its consumer is the relational frame lemma in `.design/research/relational-contracts.md` §5.1, sequenced second in that record's §10. REQ-SYNTAX-EFFECT-ROUTE is also NEW: it classifies each atom by what discharges its characteristic obligation given an honest row — NoObligation, Implemented(form), Deferred(form) or Open(question) — so `diverge` is Implemented(measures) and `blocks` is Deferred(session_duality), and an atom with no built route is named as residual trust instead of passing silently. Row honesty stays REQ-SPEC-EFFECT-ROW-CHECKED's global obligation and is deliberately outside this classification, since reading it the other way would mark every effectful function in the tree. The count therefore moves 536 → 538 requirements, 124 views unchanged, and those two additions are this entry's substantive registry changes. No status changed; all five stay not_started. `.design/reqs/status.md` is regenerated in the same change because it is a generated view over registry.toml and sits in this doc's governed set; the generator also rewrote the REQ-status regions of parser.rs, lower.rs and thermite-verified/src/lib.rs byte-identically, so no governed source moved and no other doc drifted. prior: 70bb944239ef2735f9edfd92f25e8eb55637a50b84754ac966e73bd4663896ab, previously (re-pinned 2026-08-11 for the interpreter pin: every repository gate now runs under `uv run python` from the repo root, with no system-python fallback. The gates read gates/routes.toml through tomllib (3.11+), and the system python3 on a dev machine is 3.9, so they exited on their own environment check rather than on the thing they gate: gates/audit.sh's [4'] step read that exit as `doc-drift RED - a routed design doc drifted` and returned RC=1, blaming design drift for a missing interpreter. Touches .claude/settings.json and its opt-in/ pack copy (9 hook commands each), gates/audit.sh, gates/g4.sh, and the documented invocations. No gate logic, route, or requirement changed. prior: 758aacfbaa7b0f58d21037b5fba30ad6045f24c8f65403a56389a58fb9503fcf, previously (re-pinned 2026-08-10 for the .design/tooling -> .design/gates move. I predicted this would need NO re-pins, on the correct premise that a pin digests the GOVERNED SOURCE and the source does not move. The premise held; the prediction did not, because the governed Python sources cite their own design doc path in their module docstrings - doc-drift.py says 'the detailed rules are in .design/gates/...' - so rewriting the reference changed the source and moved the digest. Second-order, same family as the glob-membership case recorded on stage4: the thing that moved was not the governed file, but something the governed file mentions. No rule and no behaviour changed; reqs check reports 536 requirements and 124 views, unchanged. Note also that req-registry's governed set includes registry.toml, whose 6 REQ-REG entries moved scope 'tooling' -> 'registry' in the same commit. prior: 12ceedc881ac20455c08bfd5ececd42e86d30b547ce8503bc7676f5ad092d79e.)))
governs:
